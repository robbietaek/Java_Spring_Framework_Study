...
public interface BoardMapper {
	...
	@Select({"<script>",
	    "select num, name, pass, subject, file1 fileurl, content, regdate, readcnt, grp, grplevel, grpstep from board ",
	    " <if test='searchtype != null'> where ${searchtype} like '%${searchcontent}%'</if>",
	    " <if test='num != null'> where num = #{num} </if>",
	    " <if test='num == null'> order by grp desc, grpstep asc limit #{startrow},#{limit}</if>",
	    "</script>"})
